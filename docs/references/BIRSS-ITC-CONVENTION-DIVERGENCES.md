...
record. All previously open verification caveats in §9 are now resolved.
...
[§9 Verification](#9-verification-status-and-method).
...
|---|---|---|---|
| **1 — Setting** | The same group embedded in the axis cross in more than one allowed way (monoclinic b-/c-unique; the 30° basal rotation of `-6m2`↔`-62m`; orthorhombic axis permutations). | **Yes** — ITC lists these too. | Component *labels* move; physics unchanged. In the app this is the **setting selector**. |
...
| **2b — Pure notation** | Spelling only: bar (`m-3` vs `m3`), Schoenflies `C₃ᵢ` vs `S₆`, prime vs underline, short vs full HM. | Mixed. | No physics. Can only trip up *symbol recognition*. |

**The key point:** the visible traps for an ITC user live in the **coupling of Level 1 and Level 2a**,
not in either alone. Example (worked in §6): the two orientations of `-6m2` are a genuine setting
...
```
 ...
 x ∥ [100]  ∥ [2-1-1 0]           (a-axis, a₁)
 y ∥ [120]  ∥ [0 1 -1 0]          (⊥ a₁ in the basal plane, 90° from x)
```
...
- **x lies along an a-axis; y does not.** The three a-axes (secondary directions `<100>`) point at
  0°, 120°, 240°. y points at 90°, which is a **tertiary** direction (`<1-10>`-type, the app's
  `[120]`). So "along y" and "along an a-axis" are *different* directions, 30°/90° apart. Under a
  bare 3-fold they are **not** equivalent — this is the whole reason trigonal groups are sensitive
  to the choice.
- **The 3-fold mixes the two crystallographic families.** Rotating an a-axis (0°) by the 3-fold
  gives 120°, 240° — still a-axes. Rotating the tertiary y (90°) gives 210°, 330° — still tertiary.
...
ITC states this explicitly. The legend of ITC Vol. D Table 1.5.2.3 reads: *"N(m⊥) denotes N mirror
planes with **normals** perpendicular to the principal symmetry axis"* — mirrors are named by their
normal. The full rule is ITC Vol. A, §2.2.4.
...
```
        ...
        mirror ⊥ a-axis (m_x)              mirror ∥ a-axis (m_y)
      ...
      ────────┼────────  x = a-axis               │      (contains x = a-axis;
              │      ╲                             │       normal ∥ y)
              │        ╲                           │
   "m at [100]" = m ⊥ a          "m normal along y" = m ∥ a
   normal on a-axis (position 2)  normal on tertiary y (Birss position 2)
```

The two frames are the **same group rotated by 30°**. They are *not* the same embedding, and for a
3-fold-only group they give **different non-zero tensor components**.

---

## 6. Worked example: `-6m2` (D₃ₕ) — the canonical case

**Group:** D₃ₕ, order 12, the symmetry of a trigonal prism. Non-centrosymmetric ⇒ electric-dipole
SHG is allowed, with exactly **one** independent rank-3 component.

**What Birss / the app do.** Birss Table 4a gives `-6m2` with generator `-2//y` (= σ(4) = [-2_y],
mirror ⊥ y). With `y ∥ [120]`, that mirror **contains the a-axis** — it is the `m_y` embedding.
Closing {6̄_z, m_y} gives the **xxx-family**:

```
   χ_yyx = χ_yxy = χ_xyy = −χ_xxx          (independent: χ_xxx)
```

**What ITC does.** ITC Vol. A Table 3.2.3.2 gives explicit coordinate triplets. For `-6m2` the
vertical-mirror operation is `x̄+y, y, z`, which in the Cartesian frame is `(x,y) → (−x, y)` = **m_x**
(mirror ⊥ a-axis). For `-62m` the corresponding operation is `x−y, ȳ, z` = `(x,y) → (x, −y)` = **m_y**.
So in ITC:

```
   ITC -6m2  = m_x embedding  →  yyy-family:  χ_xxy = χ_xyx = χ_yxx = −χ_yyy
...
> Independent literature check (orientation-agnostic count): D₃ₕ has exactly one independent ED-SHG
...
(Birss's `χ_zzz` plays the role of ITC's `χ_yyy`); the component *structure* is equivalent, only the
...
read directly from the sources in §9.
...
|---|---|---|---|---|
| Orthorhombic | D₂ₕ(C₂ₕ) | `m'm'm` (unprimed 2 ∥ z) | `mm'm'` (unprimed 2 ∥ x) | **Different default frames.** Both conventions name settings the same way; they tabulate different defaults. ITC's symbol = the app's alternate setting. |
| Hexagonal | D₆ₕ(D₃ₕ) | `6'/mm'm` | `6'/mmm'` | **Same physical frame, different string.** Birss's and ITC's default orientations coincide (unprimed mirrors ⊥ a-axes); the strings differ purely by the position-2 convention (Birss: y-family; ITC: a-axes). No component mapping is needed when comparing app output with ITC — only the label differs. |

*(A previous revision of this document listed D₃ₕ(D₃) `-6'2m'` here as a third string divergence —
that was wrong: ITC prints `-6'2m'` too (scan-verified). It belongs in §7C. D₆ₕ(D₃d) `6'/m'mm'` and
the other four D₆ₕ black-white members print identically in both conventions.)*

Note on `6'/mm'm`: the app's default frame matched Birss/ITC only from **v0.14.1** onward — before
PR #52 the app was embedded 30° off due to a transcription-pass error; see the session findings
document.
...
| Cₛ | `m` | `11m` | `1m1` |
| Cₛ(C₁) | `m'` | `11m'` | `1m'1` |
| C₂ₕ | `2/m` | `11 2/m` | `1 2/m 1` |
| C₂ₕ(Cᵢ) | `2'/m'` | `11 2'/m'` | `1 2'/m' 1` |
| C₂ₕ(C₂) | `2/m'` | `11 2/m'` | `1 2/m' 1` |
| C₂ₕ(Cₛ) | `2'/m` | `11 2'/m` | `1 2'/m 1` |
...
30° from ITC's, so the **Cartesian components that survive are different** (as in the `-6m2` xxx↔yyy
...
contain the 30°/x↔y operation that would make the two frames equivalent. Condition (ii) holds
...
*(D₃d(D₃) = `-3'm'` is the canonical Cr₂O₃ magneto-electric group; its two independent components sit
...
D₃ₕ (`-6m2`), D₃ₕ(D₃) (`-6'2m'`), D₃ₕ(C₃ᵥ) (`-6'm2'`), D₃ₕ(C₃ₕ) (`-6m'2'`), D₆(D₃) (`6'22'`),
C₆ᵥ(C₃ᵥ) (`6'mm'`), D₆ₕ(D₃d) (`6'/m'mm'`).
...
- C₂ᵥ(Cₛ) `2'm'm`: the app's operators {1, m_z, 2'_x, m'_y} equal ITC's standard positional reading
...
- D₆ₕ(C₆ᵥ) `6/m'mm`: its 6-fold unitary part populates both basal families identically, so the 30°
...
|---|---|---|---|
| Schoenflies of the `-3` family | `C₃ᵢ` | `S₆` | Same group (S₆ ≡ C₃ᵢ); HM `-3` identical. |
| Cubic bar | `m-3`, `m-3m`, `m'-3'm'` | same (bar kept) | App matches ITC here; the *barless* forms (`m3`, `m3m`) are Birss Table 6's spelling. |
| Antisymmetric mark | prime `'` | Shubnikov **underline** | e.g. ITC `4:2̲` = Birss `4:2'`. |
...
1. **Trust the short symbol, question the frame.** For trigonal/hexagonal groups the app's HM string
   ...
   the 30° rotation (x↔y-type swap of the basal indices).
...
4. **Watch the two genuine string differences** (§7A) — and note they behave oppositely:
...
5. **Ignore Level-2b spelling** (`C₃ᵢ`/`S₆`, bar, underline) — it never changes the physics.
...
- **ITC Vol. A, §2.2.4** — the HM symmetry-direction convention (mirror named by its normal).
- **Birss (1966), Table 4a** (`-6m2 = 3//z, -2//y`) and **Table 4e** (rank-3 forms, row R3,
  print-verified) via the in-repo `birss-tables/`.

**Cross-checks:** group closure of {6̄_z, m_y} vs {6̄_z, m_x} reproduces the xxx- vs yyy-family split;
independent literature (Cr₂O₃ under `-3'm'`, Fiebig *et al.* 2005) confirms the two-component
D₃d(D₃) form.
...
- **The "47".** Clarified: 47 is the number of `ALTERNATE_SETTINGS` entries in the app (groups with
  ...
  45°-setting groups (`4'mm'`, `-42m`, …) are among the 47 but are **not** Birss↔ITC divergent
...
- **Tetragonal and cubic** carry **no** orientation divergence (the 4-fold makes x ≡ y; cubic bar is
  notation only), so they are absent from §7C by design.
...
- **Birss, R. R. (1966).** *Symmetry and Magnetism.* North-Holland, Amsterdam. — Authoritative source
  ...
  `birss-tables/`).
- **International Tables for Crystallography, Vol. A (2016), Ch. 3.2** — Hahn, Klapper, Müller &
  Aroyo, *Point groups and crystal classes*, pp. 720–776. Table 3.2.3.2 (coordinate triplets, special
  projections); §2.2.4 (HM symmetry directions).
...
- **2026-07-04 (Revision 2, after v0.14.1 / PR #52):** Corrected §7A — removed the erroneous
  `-6'2m'` string-divergence row (ITC prints `-6'2m'`; moved to §7C) and reclassified
  `6'/mm'm`/`6'/mmm'` as *same frame, different string* (position-convention artefact; the app
  matches this frame from v0.14.1). Corrected §7C — added D₃ₕ(D₃), removed `2'm'm` and `6/m'mm`
  (both verified non-divergent), upgraded the verification status to the permanent 58-row Table-7
  audit, added the grey-derivative note. Updated §8 guidance and §9: the "~47" resolved as the
