...
Worked reference: `3m`, rank 4, polar, i-type, `none` → 14 independent components, displayed as three pair equalities, `χ_xxxx = χ_yyyy`, eleven z-block chains, and the one composite above (each partner rendered as the canonical, lowest-index member of its class). Evidence and verification record: `docs/findings/FINDING-2026-07-29-rank4-trigonal-hexagonal-overcount.md`.
