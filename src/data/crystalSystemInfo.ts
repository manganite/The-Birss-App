...
export const CRYSTAL_SYSTEM_INFO: Record<string, CrystalSystemInfo> = {
  ...
  Monoclinic: {
    lattice:
      '\\text{One oblique angle: }\\alpha = \\beta = 90^\\circ,\\ \\gamma\\ \\text{free (c-unique, Birss) -- or }\\alpha = \\gamma = 90^\\circ,\\ \\beta\\ \\text{free (b-unique, ITC).}',
    definingSymmetry: 'Exactly one order-2 direction (a 2-fold axis and/or mirror normal): the unique axis.',
    hmPositions:
      'One essential position -- the unique axis. Full symbols make it explicit: 112 (c-unique) vs 121 (b-unique).',
  ...
  Orthorhombic: {
    ...
    settings:
      'Which token sits on which axis is a choice: up to three frames (c-/a-/b-unique). Frame symbols are convention-independent.',
  ...
  Trigonal: {
    ...
    birssVsItc:
      'ITC reads position 2 from the a-axes (x); Birss from y -- 30° apart. The same symbol names two rotated frames; components appear as the xxx- vs yyy-family.',
    holohedry: '-3m',
    example: 'quartz (SiO2)',
  },
  Hexagonal: {
    ...
    birssVsItc:
      "ITC reads position 2 from the a-axes (x); Birss from y -- 30° apart. The same symbol names two rotated frames; components appear as the xxx- vs yyy-family. For one group even the string differs (6'/mm'm ↔ 6'/mmm').",
    holohedry: '6/mmm',
    example: 'graphite',
  },
  Cubic: {
