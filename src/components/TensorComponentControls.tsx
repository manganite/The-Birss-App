...
const MAGNITUDE_DETENTS = [
  ...
  ...[0.05, 0.1, 0.15, 0.2, 0.3, 0.35, 0.4, 0.45, 0.55, 0.6, 0.65, 0.7, 0.8, 0.85, 0.9, 0.95].map((val) => ({
    val,
    threshold: 0.01,
  })),
];
...
export function TensorComponentControls({
  ...
  useEffect(() => {
    if (
      ...
      (!mobileActiveComponent || !independentComponents.includes(mobileActiveComponent))
    ) {
    ...
    <div className="space-y-6">
          ...
          <div className="flex flex-wrap gap-1.5">
            {independentComponents.map((comp) => (
              ...
              >
                ...
                <span className="font-mono text-xs opacity-70">{(amplitudes[comp] ?? 1).toFixed(2)}</span>
              </button>
            ))}
          </div>
                ...
                <div className={`bg-white/50 border border-ink p-3 space-y-1.5 ${singleComponent ? 'opacity-40' : ''}`}>
                  {singleComponent && (
                    <div className="flex items-start gap-2 p-2 border border-ink/10 bg-ink/5 text-xs leading-relaxed opacity-70 mb-2">
                      ...
                      <span>
                        Single component: amplitude is only a global scale; the phase is unobservable in intensity.
                      </span>
                    </div>
                  )}
          ...
          <>
            ...
            {independentComponents.map((comp) => {
              ...
              return (
                <div
                  key={comp}
                  className={`space-y-1.5 border-b border-ink/10 pb-3 last:border-0 last:pb-0 ${singleComponent ? 'opacity-40' : ''}`}
                >
                  <div className="flex items-center gap-2">
      ...
      {(selectedGroup.crystalSystem === 'Triclinic' || selectedGroup.crystalSystem === 'Monoclinic') && (
        <div className="hidden md:block p-3 border border-ink border-opacity-10 bg-ink/5 text-xs opacity-70 leading-relaxed">
          Component values and polarimetry orientations depend on the in-plane Cartesian convention (see Help).
          Different unit-cell angles (beta for monoclinic; alpha, beta, gamma for triclinic) are represented by
          adjusting component values, not a separate control. Birefringence is not modeled.
        </div>
      )}
    </div>
  );
