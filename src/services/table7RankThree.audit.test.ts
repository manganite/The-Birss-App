...
const HDR = ['xxx', 'yyy', 'zzz', 'xxy(3)', 'yyx(3)', 'xxz(3)', 'yyz(3)', 'zzx(3)', 'zzy(3)', 'xyz', 'xzy', 'zxy', 'yxz', 'yzx', 'zyx'];
const perm3 = (s: string) => { const [a, b, c] = s.split(''); return [...new Set([a + b + c, a + c + b, c + a + b])]; };
...
function parseTable7(): Row[] {
  ...
  t7.split('\n').forEach((line, li) => {
    ...
    rows.push({
      line: li + 1, M: M.sym, Mbr: M.br, A: A.sym, Abr: A.br, B: B.sym, Bbr: B.br,
...
const inImage = (P: { apply: (v: Vec) => Vec }, v: Vec) => {
  const w = P.apply(v);
  let d = 0, n = 0;
  for (let i = 0; i < 27; i++) { d += (w[i] - v[i]) ** 2; n += v[i] ** 2; }
...
describe('Table 7 rank-3 audit (58 BW rows) -- print-pattern exceptions', () => {
  ...
  for (const row of ROWS) {
    ...
    (['i_pol', 'i_ax', 'c_pol', 'c_ax'] as CellCol[]).forEach(col => {
      ...
      const E = FORMS[cell.letter];
      const shouldBr = src.br && !!T && !sameSpan(E, E.map(v => matVec27(T, v)));
      if (cell.br !== shouldBr) {
