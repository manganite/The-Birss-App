...
import {
  isPolar, isFerromagnetic, isMagnetoelectric, isPiezoelectric, isPiezomagnetic,
  isChiral, getLaueClass, getSHGConsequenceShort,
} from './propertyFlags';
...
describe('Piezomagnetic == ITC Table 1.5.7.1 (nonzero forms, non-grey groups)', () => {
  ...
  for (const block of content.split(/^## Block /m).slice(1)) {
    ...
    const text = block.slice(gi, fence === -1 ? undefined : fence).replace(/^Groups:\s*/, '').replace(/\n/g, ' ');
    // Neutralize the ';' inside bracketed alternates (e.g. `mm2 [2mm; m2m]`) before entry-splitting.
    const safe = text.replace(/\[[^\]]*\]/g, b => b.replace(/;/g, ','));
    for (const entry of safe.split(';').map(s => s.trim()).filter(Boolean)) {
